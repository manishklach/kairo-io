...
record_summary() {
  {
    ...
    echo "stage20_dryrun=${SUMMARY[stage20_dryrun]}"
...
# Stage 20 dry-run
if [[ -f "$REPO_ROOT/scripts/run_stage20_kv_admission_experiment.sh" ]]; then
  if $DRY_RUN; then
    printf '%s\n' "./scripts/run_stage20_kv_admission_experiment.sh \"$TEST_FILE\" loop0 --skip-counters --dry-run --duration \"$DURATION\"" > "$RESULTS_DIR/stage20_dryrun.log"
    SUMMARY[stage20_dryrun]="pass"
  else
    if run_logged "$RESULTS_DIR/stage20_dryrun.log" "$REPO_ROOT/scripts/run_stage20_kv_admission_experiment.sh" \
      "$TEST_FILE" loop0 --skip-counters --dry-run --duration "$DURATION"; then
      SUMMARY[stage20_dryrun]="pass"
    else
      SUMMARY[stage20_dryrun]="fail"
    fi
  fi
else
  echo "run_stage20_kv_admission_experiment.sh not found" > "$RESULTS_DIR/stage20_dryrun.log"
  SUMMARY[stage20_dryrun]="missing_script"
fi
...
# Stage 18 dry-run
if [[ -f "$REPO_ROOT/scripts/run_stage18_recompute_eviction_experiment.sh" ]]; then
  if $DRY_RUN; then
    printf '%s\n' "./scripts/run_stage18_recompute_eviction_experiment.sh \"$TEST_FILE\" loop0 --skip-counters --dry-run --duration \"$DURATION\"" > "$RESULTS_DIR/stage18_dryrun.log"
    SUMMARY[stage18_dryrun]="pass"
  else
    if run_logged "$RESULTS_DIR/stage18_dryrun.log" "$REPO_ROOT/scripts/run_stage18_recompute_eviction_experiment.sh" \
      "$TEST_FILE" loop0 --skip-counters --dry-run --duration "$DURATION"; then
      SUMMARY[stage18_dryrun]="pass"
    else
      SUMMARY[stage18_dryrun]="fail"
    fi
  fi
else
  echo "run_stage18_recompute_eviction_experiment.sh not found" > "$RESULTS_DIR/stage18_dryrun.log"
  SUMMARY[stage18_dryrun]="missing_script"
fi
